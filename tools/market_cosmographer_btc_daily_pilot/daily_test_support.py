...
def policy():
    return {'schema_version': 'market_cosmographer_btc_daily_utility_pilot_policy_v0_1', 'pilot_id': 'market_cosmographer_btc_30_day_utility_pilot_v0_1', 'status': 'AUTHORIZED_INTERNAL_PILOT', 'start_observation_date': '2026-07-26', 'end_observation_date': '2026-08-24', 'planned_consecutive_days': 30, 'subject': {'asset_id': 'bitcoin', 'symbol': 'BTC', 'market': 'BTCUSDT_SPOT', 'interval': '1d_UTC'}, 'source_policy': {'provider': 'BINANCE_PUBLIC_DATA', 'archive_frequency': 'daily', 'source_window_days': 32, 'checksum_required': True, 'raw_archive_distribution': False, 'repository_storage': False}, 'freshness_policy': {'freshness_policy_id': 'completed_utc_daily_snapshot_36h_v0_1', 'fresh_max_age_hours': 36, 'aging_max_age_hours': 72, 'pilot_accepts_only': 'FRESH'}, 'accepted_product_fields': {'tier2_metrics': list(pilot.TIER2_METRICS), 'tier3_labels': ['range_state']}, 'automated_daily_gates': [], 'manual_utility_questions': [], 'completion_thresholds': {}, 'distribution': {}}
...
def write_daily_archive(folder: Path, day: date, index: int) -> None:
    name = pilot.archive_name(day)
    member = pilot.member_name(day)
    close = 50000.0 + index * 131.0 + index % 5 * 17.0
    open_value = close - 75.0
    high = close + 250.0
    low = close - 300.0
    base_volume = 1000.0 + index * 4.0
    ...
    with zipfile.ZipFile(buffer, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr(member, text.getvalue().encode('utf-8'))
    payload = buffer.getvalue()
    digest = hashlib.sha256(payload).hexdigest()
    (folder / name).write_bytes(payload)
    (folder / f'{name}.CHECKSUM').write_text(f'{digest}  {name}\n', encoding='utf-8')
...
def make_window(folder: Path, start: date, end: date) -> None:
    ...
    index = 0
    while day <= end:
        write_daily_archive(folder, day, index)
...
class DailyPilotBase(unittest.TestCase):
    def setUp(self):
        self.policy = pilot.validate_policy(policy())
        self.observation = date(2026, 7, 26)
        self.start = self.observation - timedelta(days=31)
        self.generated = '2026-07-27T06:00:00Z'

    def build(self, root: Path, previous_manifest=None):
        archive_dir = root / 'archives'
        output_dir = root / 'output'
        make_window(archive_dir, self.start, self.observation)
        rows, manifest = pilot.read_source_window(archive_dir, self.start, self.observation, self.generated)
        correction = pilot.build_correction_ledger(manifest, previous_manifest, self.observation)
        ...
        report = pilot.write_build_outputs(output_dir, packet, state, manifest, correction, proof, diagnostics, self.policy, FakeValidator, root)
        return (archive_dir, output_dir, packet, report, manifest)
