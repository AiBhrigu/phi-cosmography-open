...
<section id="surface" data-editorial-chapter="orientation"><a href="https://www.bhrigu.io/crypto-astro/btc">OPEN BTC FIELD</a></section>
...
<details id="proof"><summary>Proof</summary></details>
    ...
    def test_accepts_separate_brand_anchor(self) -> None:
        value = self.sample().replace(
            ...
            '<nav class="crypto-astro-primary-nav"><a class="crypto-astro-primary-nav__brand" href="#surface">Market Cosmographer</a>',
            1,
        )
        report = verify(value)
        self.assertEqual(report["status"], "PASS", report["failures"])
    ...
    def test_rejects_competing_primary_cta(self) -> None:
        value = self.sample().replace("</section>\n<section id=\"what-changed\"", '<a href="https://www.bhrigu.io/crypto-astro/btc/clean-chat?lang=en">Ask one BTC field question</a></section>\n<section id="what-changed"', 1)
        report = verify(value)
        self.assertEqual(report["status"], "FAIL")
        self.assertIn("btc_entry_route_split", report["failures"])
